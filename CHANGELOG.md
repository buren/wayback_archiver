...
- CLI output reads from the reader's side: the progress footer's `Polling...` is now `Waiting for captures to finish...` (with `Submitting URLs...` and `Waiting for a free capture slot...` alongside it), and the auto strategy no longer logs its expected "no sitemap here" probe at ERROR — it reports `No Sitemap found at <url> (HTTP 404) - falling back to crawling` at info level.
