...
- Screenshot downloads no longer save HTTP error bodies as `.png` files. `Screenshot.download` fetched without `raise_on_http_error`, so a 404 from the screenshot endpoint was written to disk as a PNG, logged as "Screenshot saved to ...", and recorded in the report. It now raises on HTTP errors, verifies the PNG signature before writing, and sends the Internet Archive credentials it already required you to configure.
